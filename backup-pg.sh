...
if [ ($(echo "$AV_SPACE < 18" | bc) -eq 1 ] # check available space and if it greater than 18G do backup without removing one old buckup file
	then
		...
		rm -f $(find . -name "dumpall_*" -type f -printf '%T@+ %p\n' | sort -n | head -n 1) # remove oldest backup file if their number is exceeded
		slug=$(date +"%T_%d_%m_%Y")
